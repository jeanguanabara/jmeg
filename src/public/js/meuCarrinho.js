window.addEventListener("load", () => {
  ...
  for (let i in somarItemCarrinho) {
    somarItemCarrinho[i].addEventListener("click", (event) => {
        ...
        console.log(parseInt(qntItemCarrinho[i].value))
      let qntAtualItemCarrinho = parseInt(qntItemCarrinho[i].value);

      qntAtualItemCarrinho += 1;

      qntItemCarrinho[i].value = String(qntAtualItemCarrinho);
      calculaVlrTotal();
  ...
  function calculaVlrTotal() {
    ...
    for (let i = 0; i < qntItemCarrinho.length; i++) {
      ...
      console.log("let i = ", i);
      let qnt = parseInt(qntItemCarrinho[i].value) ;
      ...
      vlrTotal += qnt * vlr;
      console.log(
        "Meio de função é dentro do for, vair ser o valor até essa execução = ",
        vlrTotal
      );
    }
